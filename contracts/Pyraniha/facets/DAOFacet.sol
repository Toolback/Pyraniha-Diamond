...
import {Modifiers, ItemType, WearableSet, NUMERIC_TRAITS_NUM, EQUIPPED_WEARABLE_SLOTS, PyranihaCollateralTypeInfo} from "../libraries/LibAppStorage.sol";
import {PyranihaCollateralTypeIO} from "../libraries/LibPyraniha.sol";
...
contract DAOFacet is Modifiers {
    ...
    event AddCollateralType(PyranihaCollateralTypeIO _collateralType);
    event AddItemType(ItemType _itemType);
    event CreateCycle(uint256 indexed _cycleId, uint256 _cycleMaxSize, uint256 _eggPrice, bytes32 _bodyColor);
    event CreateUnion(uint256 indexed _unionId, uint256 _unionMaxSize, uint256 _companyPrice, bytes32 _bodyColor);
   ...
   |__________________________________*/
    ...
    function addCollateralTypes(uint256 _cycleId, PyranihaCollateralTypeIO[] calldata _collateralTypes) public onlyItemManager {
        for (uint256 i; i < _collateralTypes.length; i++) {
            ...
            for (uint256 cycleIndex = 0; cycleIndex < s.cycleCollateralTypes[_cycleId].length; cycleIndex++) {
                ...
                if (existingCycleCollateral == newCollateralType) {
    ...
    function createCycle(
        uint24 _cycleMaxSize,
        uint96 _eggPrice
    ) external onlyDaoOrOwner returns (uint256 cycleId_) {
        uint256 currentCycleId = s.currentCycleId;
        require(
            s.cycles[currentCycleId].totalCount == s.cycles[currentCycleId].cycleMaxSize,
            "PyranihaFacet: Cycle must be full before creating new"
        );
        cycleId_ = currentCycleId + 1;
        s.currentCycleId = uint16(cycleId_);
        s.cycles[cycleId_].cycleMaxSize = _cycleMaxSize;
        s.cycles[cycleId_].eggPrice = _eggPrice;
        emit CreateCycle(cycleId_, _cycleMaxSize, _eggPrice);
    }

    struct CreateCyclePayload {
        uint24 _cycleMaxSize;
        uint96 _eggPrice;
        PyranihaCollateralTypeIO[] _collateralTypes;
    ...
    function createCycleWithPayload(CreateCyclePayload calldata _payload) external onlyItemManager returns (uint256 cycleId_) {
        uint256 currentCycleId = s.currentCycleId;
        require(
            s.cycles[currentCycleId].totalCount == s.cycles[currentCycleId].cycleMaxSize,
            "PyranihaFacet: Cycle must be full before creating new"
        ...
        addCollateralTypes(cycleId_, _payload._collateralTypes);
        ...
        s.currentCycleId = uint16(cycleId_);
        s.cycles[cycleId_].cycleMaxSize = _payload._cycleMaxSize;
        s.cycles[cycleId_].eggPrice = _payload._eggPrice;
        emit CreateCycle(cycleId_, _payload._cycleMaxSize, _payload._eggPrice);
    }
    ...
    function createUnion(
        uint24 _unionMaxSize,
        uint96 _CompanyPrice
    ) external onlyDaoOrOwner returns (uint256 unionId_) {
        uint256 currentUnionId = s.currentUnionId;
        require(
            s.unions[currentUnionId].totalCount == s.unions[currentUnionId].unionMaxSize,
            "PyranihaFacet: Last Union must be full before creating new"
        );
        unionId_ = currentUnionId + 1;
        s.currentUnionId = uint16(unionId_);
        s.unions[unionId_].unionMaxSize = _unionMaxSize;
        s.unions[unionId_]._companyPrice = _companyPrice;
        emit CreateUnion(unionId_, _unionMaxSize, _companyPrice);
    }
    ...
    function mintItems(
        ...
        for (uint256 i; i < _itemIds.length; i++) {
            ...
            require(s.itemTypes[itemId].category != 3, "DAOFacet: Cant mint Company !");
